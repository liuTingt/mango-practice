<template>
    <div class="headbar" :style="{'background':themeColor}" :class="collapse?'position-collapse-left':'position-left'">
        <!-- 导航收缩 -->
        <span class="hamburg">
            <!-- 将各个页面中需要同步为主题色的页面或组件绑定store中的主题色属性themeColor -->
        ...
        <!-- 导航菜单 -->
        <span class="navbar">
            <el-menu :default-active="activeIndex" class="el-menu-demo" :background-color="themeColor"
...
import { mapState } from 'vuex'  // 使用Store
...
export default {
    ...
    mounted() {
        ...
        if(user){// 先登录，将用户信息保存在cookie中
            this.user.name = user;
            this.user.avatar = require("@/assets/user2.png");
        }
    },
    // 计算属性
    computed: {
        ...mapState({
            themeColor: state => state.app.themeColor, // 加入主题生成默认颜色
